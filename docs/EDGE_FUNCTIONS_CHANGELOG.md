...
## [2025-12-01] - 🎯 TMWE Functions Consolidation (4→1)

### 🎯 Resumen Ejecutivo
- ✅ **Consolidated 4 TMWE functions** into `tmwe-api-proxy` as internal handlers
- ✅ **Backups created** in `docs/EDGE_FUNCTIONS_BACKUP/2025-12-01/`
- ✅ **Frontend updated** (7 files) to invoke `tmwe-api-proxy` with handler parameter
- ✅ **Original functions preserved** (will be disabled after testing)
- 📊 **Impact**: 6 → 2 TMWE functions (4 slots saved)

### Functions Consolidated

| Original Function | New Handler Name | Purpose |
|-------------------|-----------------|---------|
| `tmwe-oauth-auth` | `oauth_auth` | OAuth2 authorization code exchange |
| `tmwe-oauth-refresh` | `oauth_refresh` | OAuth2 token refresh |
| `tmwe-email-send` | `email_send` | Send emails via TMWE API |
| `tmwe-email-sync-master` | `email_sync_master` | Batch email synchronization |

### Files Modified

#### Edge Functions
- **`supabase/functions/tmwe-api-proxy/index.ts`** - Added 4 new handlers (~750 lines)

#### Frontend Files (7)
- **`src/pages/OAuthCallback.tsx`** - Changed `tmwe-oauth-auth` → `tmwe-api-proxy` (handler: oauth_auth)
- **`src/components/email/EmailManagementTab.tsx`** - Changed `tmwe-email-sync-master` → `tmwe-api-proxy` (handler: email_sync_master)
- **`src/lib/email/services/EdgeFunctionSyncService.ts`** - Changed `tmwe-email-sync-master` → `tmwe-api-proxy` (handler: email_sync_master)
- **`src/components/email/EmailComposer.tsx`** - Changed `tmwe-email-send` → `tmwe-api-proxy` (handler: email_send)
- **`src/pages/ImportTemplates.tsx`** - Changed `tmwe-email-send` → `tmwe-api-proxy` (handler: email_send)
- **`supabase/functions/_shared/oauth-manager.ts`** - Changed `tmwe-oauth-refresh` → `tmwe-api-proxy` (handler: oauth_refresh)
- **`src/lib/tmwe-api-integrated.ts`** - Changed `tmwe-oauth-refresh` → `tmwe-api-proxy` (handler: oauth_refresh)

### Backups Created
```
docs/EDGE_FUNCTIONS_BACKUP/2025-12-01/
├── tmwe-oauth-auth_backup.ts
├── tmwe-oauth-refresh_backup.ts
├── tmwe-email-send_backup.ts
└── tmwe-email-sync-master_backup.ts
```

### Architecture After Consolidation
```
┌──────────────────────────────────────────────────────────┐
│                  TMWE EDGE FUNCTIONS (2)                 │
├──────────────────────────────────────────────────────────┤
│  1. tmwe-api-proxy (GATEWAY)                             │
│     ├─ oauth_auth           ← OAuth authorization       │
│     ├─ oauth_refresh         ← Token refresh            │
│     ├─ email_send            ← Send emails              │
│     ├─ email_sync_master     ← Batch sync               │
│     ├─ internal_jwt_sign     ← JWT signing (existing)   │
│     ├─ internal_jwt_refresh  ← JWT refresh (existing)   │
│     └─ [proxy genérico]      ← General TMWE API calls   │
│                                                          │
│  2. tmwe-jwt-sign (MIGRATION)                            │
│     └─ migrate_email_ids     ← Email ID migration       │
└──────────────────────────────────────────────────────────┘
```

### Rollback Plan
```bash
# Restore individual function from backup
cp docs/EDGE_FUNCTIONS_BACKUP/2025-12-01/[function]_backup.ts supabase/functions/[function]/index.ts
...
### Testing Checklist
- [ ] OAuth login flow (OAuthCallback → oauth_auth handler)
- [ ] Token refresh (oauth-manager → oauth_refresh handler)
- [ ] Email sync (EmailManagementTab → email_sync_master handler)
- [ ] Email send (EmailComposer → email_send handler)
- [ ] Verify logs in Supabase Dashboard

### Next Steps
1. **Deploy** and test all handlers
2. **Verify** OAuth flow, sync, and send operations
3. **Move** original functions to `supabase/functions/_disabled/` (only after successful testing)
4. **Update** `supabase/config.toml` to remove disabled functions

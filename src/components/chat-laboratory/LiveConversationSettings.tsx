...
interface LiveConversationSettingsProps {
...
export const LiveConversationSettings = ({ conversationId, onSettingsChange }: LiveConversationSettingsProps) => {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (data) {
        console.log('🎙️ LiveConversationSettings - Caricato da DB:', data);
        setSettings(data as LiveSettings);
        setPreset((data.preset as any) || 'professional');
        onSettingsChange?.(data as LiveSettings);
      }
    } catch (error) {
  ...
  const updateSetting = async <K extends keyof LiveSettings>(
    key: K,

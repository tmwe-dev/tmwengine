...
export const EconomyModeToggle = ({ conversationId, onSettingsChange }: EconomyModeToggleProps) => {
  ...
  const updateSetting = async (field: 'economy_mode' | 'show_summaries_only', value: boolean) => {
    if (!conversationId) {
      ...
      localStorage.setItem(`chat_${field}`, value.toString());
    ...
    <div className="flex flex-col gap-3 p-4 border rounded-lg bg-card/50">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <Label htmlFor="economy-mode-chat" className="text-sm font-medium cursor-pointer">
            Economy Mode
          </Label>
        </div>
        <Switch
          id="economy-mode-chat"
          checked={economyMode}
      ...
      <div className="flex items-center justify-between gap-3 pt-2 border-t">
        <div className="flex items-center gap-2">
          ...
          <Label htmlFor="show-summaries-chat" className="text-sm font-medium cursor-pointer">
            Mostra Riassunti
          </Label>
        </div>
        <Switch
          id="show-summaries-chat"

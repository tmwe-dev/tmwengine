...
interface RadioAudioSettingsPopupProps {
  conversationId: string | null;
  onConfigChange?: (config: AudioConfig) => void;
}

export const RadioAudioSettingsPopup = ({ conversationId, onConfigChange }: RadioAudioSettingsPopupProps) => {
  const [selectedConfig, setSelectedConfig] = useState<AudioConfig>('standard');
  const [isOpen, setIsOpen] = useState(false);

  useEffect(() => {
    const saved = localStorage.getItem('radio-audio-config');
    if (saved) {
  ...
  const saveAudioConfig = (config: AudioConfig) => {
    ...
    localStorage.setItem('radio-audio-config', config);

...
interface RadioSidebarProps {
  ...
  /** When true, renders as pure content without fixed wrapper/backdrop/header */
  embedded?: boolean;
}
...
export function RadioSidebar({ 
  ...
  // Embedded mode: pure content, no wrapper/backdrop/header
  if (embedded) {
    return (
  ...
  // Standalone mode: full fixed sidebar with backdrop
  return (
    <>
      {isOpen && (
        <div 

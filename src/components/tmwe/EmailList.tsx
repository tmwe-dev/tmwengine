...
export const EmailList = ({ 
  ...
  const renderListView = () => (
    <div className={cn("space-y-2 py-4 px-4 sm:px-6 w-full mx-auto overflow-x-hidden", getWidthClass())}>
      {filteredEmails.map((email, index) => (
        <Card
          key={email.id}
          ref={index === filteredEmails.length - 1 ? lastEmailRef : null}
          className={cn(
            'relative cursor-pointer border-l-4 p-0 overflow-hidden transition-colors transition-shadow transition-transform duration-200 w-[75%] mx-auto',
  ...
  const renderGridView = () => (
    <div className={cn("space-y-1.5 sm:space-y-2 py-1.5 sm:py-2 px-1.5 sm:px-2 w-full mx-auto overflow-x-hidden", getWidthClass())}>
      {filteredEmails.map((email, index) => (
        <Card
          key={email.id}
          ref={index === filteredEmails.length - 1 ? lastEmailRef : null}
        ...
        >
          <div className="flex items-stretch min-w-0 w-full">
            ...
            <div className="flex-1 p-1.5 sm:p-2 md:p-4 relative">
              <div className="flex items-start justify-between gap-1.5 sm:gap-2 md:gap-4 h-full min-w-0">

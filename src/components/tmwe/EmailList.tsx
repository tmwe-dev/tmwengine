...
export const EmailList = ({ 
  ...
  const renderListView = () => (
    <div className={cn("space-y-2 py-4 px-4 sm:px-6 w-full overflow-x-hidden", getWidthClass())}>
      {filteredEmails.map((email, index) => (
        <Card
          key={email.id}
          ref={index === filteredEmails.length - 1 ? lastEmailRef : null}
          className={cn(
            'relative cursor-pointer border-l-4 p-0 overflow-hidden transition-colors transition-shadow transition-transform duration-200 w-[75%] ml-0',
  ...
  const renderGridView = () => (
    <div className={cn("space-y-1.5 sm:space-y-2 py-1.5 sm:py-2 px-1.5 sm:px-2 w-full overflow-x-hidden", getWidthClass())}>
      {filteredEmails.map((email, index) => (
        <Card
          key={email.id}
          ref={index === filteredEmails.length - 1 ? lastEmailRef : null}

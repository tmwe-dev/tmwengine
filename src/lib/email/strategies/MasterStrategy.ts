...
import { downloadSingleEmail } from '../email-downloader';
import type {
  DownloadStrategy,
  StrategyConfig,
  DownloadProgress,
...
export class MasterStrategy implements DownloadStrategy {
  ...
  async execute(
    ...
    try {
      for (const folder of folders) {
        ...
        const startUID = folder.startUID || maxUID + 1;

        onLog({
          phase: 'preparing',
          folder: folder.folderName,
          message: `📦 Starting from UID ${startUID} (DB has ${maxUID})`
        });
        ...
        while (emptyBatches < MAX_EMPTY) {
          ...
          onLog({
            phase: 'importing',
            folder: folder.folderName,
            message: `⬇️ Downloading batch: UIDs ${batch[0]}-${batch[batch.length - 1]}`
          });
  ...
  private async downloadBatchWithProtection(
    ...
    for (const uid of uids) {
      ...
      try {
        // A. Download con CircuitBreaker + Timeout
        const result = await this.breaker.execute(async () => {
          return await this.downloadSingleEmailWithTimeout(
            uid,
            folder,
            userEmail,
            30000 // 30s timeout
        ...
        // B. Circuit breaker OPEN? Log and pause
        ...
        // C. Add to persistent queue (crash recovery)
        if (error.message?.includes('timeout') || error.message?.includes('network')) {
          ...
          onLog({
            phase: 'warning',
            folder,
            message: `💾 UID ${uid} added to recovery queue (${error.message})`
          });
        }
      }
    }

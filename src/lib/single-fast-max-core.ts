...
export async function processFolderWithDance(
  ...
  try {
    while (hasMore) {
      ...
      // 1. ✅ Fetch UIDs batch piccolo dall'API (NO pre-popolazione)
      const uids = await fetchUIDsBatchFromAPI(folder, page, full_config.batch_size);
      ...
      console.log(`[processFolderWithDance] ✅ Fetched ${uids.length} UIDs from API`);
      
      // 2. ✅ Filtra UIDs già importati (check DB locale, velocissimo)
      const pendingUIDs = await filterPendingUIDs(uids, folder, user_email);
      ...
      // 3. ✅ Download IMMEDIATO di questi UIDs (max 2 paralleli)
      ...
      // 4. ✅ Delay prima prossima pagina (riduce carico Edge Function)
      page++;
...
async function fetchUIDsBatchFromAPI(
  ...
  try {
    ...
    console.error(`[fetchUIDsBatchFromAPI] ❌ Error fetching UIDs:`, error.message);
    return [];
  }
}

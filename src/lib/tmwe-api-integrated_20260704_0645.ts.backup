...
export const initiateAuthorizationCodeFlow = (): void => {
  ...
  const authUrl = new URL('https://sandbox.findair.net/erp/tmwe_json/auth'); // NO /authorization
...
const generateClientJWT = (clientId: string, clientSecret: string): string => {
  ...
  const payload = {
    ...
    aud: 'https://sandbox.findair.net/erp/tmwe_json/token', // Audience (token endpoint)
...
export const authenticateWithJWT = async (): Promise<boolean> => {
  ...
  try {
    ...
    const response = await fetch('https://sandbox.findair.net/erp/tmwe_json/token', {
      method: 'POST',
      headers: {

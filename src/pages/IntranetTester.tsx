...
import { MessageSquare, Play, Download, Lightbulb } from 'lucide-react';
import { TestRunner } from '@/lib/testing/test-runner';
import { intranetTestSuites } from '@/lib/testing/intranet-tests';
...
export const IntranetTester = () => {
  const [selectedSuite, setSelectedSuite] = useState(intranetTestSuites[0]?.name || '');
  ...
  const handleRunTests = async () => {
    const suite = intranetTestSuites.find(s => s.name === selectedSuite);
    if (!suite) return;
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `intranet-test-report-${Date.now()}.json`;
    a.click();
  };
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Test performance chat, presenza utenti e gestione stanze
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <TestSuiteSelector
            suites={intranetTestSuites}

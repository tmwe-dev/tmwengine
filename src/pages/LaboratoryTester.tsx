...
import { Sparkles, Play, Download, Lightbulb } from 'lucide-react';
import { TestRunner } from '@/lib/testing/test-runner';
import { laboratoryTestSuites } from '@/lib/testing/laboratory-tests';
...
export const LaboratoryTester = () => {
  const [selectedSuite, setSelectedSuite] = useState(laboratoryTestSuites[0]?.name || '');
  ...
  const handleRunTests = async () => {
    const suite = laboratoryTestSuites.find(s => s.name === selectedSuite);
    if (!suite) return;
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `laboratory-test-report-${Date.now()}.json`;
    a.click();
  };
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Test microfono, AI agents, orchestratore e prompt sections
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <TestSuiteSelector
            suites={laboratoryTestSuites}

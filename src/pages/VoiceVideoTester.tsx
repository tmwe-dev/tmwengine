...
import { Video, Play, Download, Lightbulb } from 'lucide-react';
import { TestRunner } from '@/lib/testing/test-runner';
import { voiceVideoTestSuites } from '@/lib/testing/voice-video-tests';
...
export const VoiceVideoTester = () => {
  const [selectedSuite, setSelectedSuite] = useState(voiceVideoTestSuites[0]?.name || '');
  ...
  const handleRunTests = async () => {
    const suite = voiceVideoTestSuites.find(s => s.name === selectedSuite);
    if (!suite) return;
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `voice-video-test-report-${Date.now()}.json`;
    a.click();
  };
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Voice & Video Performance Testing
          </CardTitle>
          <CardDescription>
            Test comunicazione voce, preparazione video e diagnostica WebRTC
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <TestSuiteSelector
            suites={voiceVideoTestSuites}

-- Create RPC function to safely get data from temporary import tables
...
DECLARE
    ...
    query_text text;
BEGIN
    -- Validate table name starts with 'import_temp_' for security
    IF table_name IS NULL OR NOT table_name LIKE 'import_temp_%' THEN
        RAISE EXCEPTION 'Invalid table name. Must start with import_temp_';
    END IF;

DROP FUNCTION IF EXISTS public.get_tables_with_counts();
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN t.table_name = 'import_errors' THEN (SELECT COUNT(*) FROM import_errors)
      ...
      WHEN t.table_name = 'intranet_messages' THEN (SELECT COUNT(*) FROM intranet_messages)
      WHEN t.table_name = 'intranet_room_members' THEN (SELECT COUNT(*) FROM intranet_room_members)
      WHEN t.table_name = 'intranet_rooms' THEN (SELECT COUNT(*) FROM intranet_rooms)
      WHEN t.table_name = 'page_system_prompts' THEN (SELECT COUNT(*) FROM page_system_prompts)
      ...
      WHEN t.table_name = 'email_sender_actions' THEN (SELECT COUNT(*) FROM email_sender_actions)
      ELSE 0
    END as row_count

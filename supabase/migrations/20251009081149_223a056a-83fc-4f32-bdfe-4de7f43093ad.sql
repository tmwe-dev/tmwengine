-- Passo 2: Inviti, Timeout, Badge Personalizzati

-- Aggiungi colonna per inviti proattivi
ALTER TABLE public.intranet_room_access_requests
ADD COLUMN is_invite boolean NOT NULL DEFAULT false,
ADD COLUMN invited_by uuid REFERENCES auth.users(id);

-- Aggiungi badge personalizzati per stati utente
ALTER TABLE public.user_profiles
ADD COLUMN status_emoji text,
ADD COLUMN status_color text DEFAULT '#10b981';
...
BEGIN
  ...
  -- Se è una stanza pubblica e non è un invito, approva automaticamente
  IF room_access_type = 'public' AND NEW.is_invite = false THEN
    NEW.status := 'approved';
    NEW.reviewed_at := now();
...
-- Policy aggiuntiva: gli admin possono creare inviti
CREATE POLICY "Admins can create invites"
  ON public.intranet_room_access_requests
  FOR INSERT
  TO authenticated
  WITH CHECK (
    is_admin(auth.uid()) AND is_invite = true
  );

-- Policy aggiuntiva: i creatori delle stanze possono creare inviti
CREATE POLICY "Room creators can create invites"
  ON public.intranet_room_access_requests
  FOR INSERT
  TO authenticated
  WITH CHECK (
